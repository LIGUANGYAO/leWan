    ...
    var calendarSwitch = (function() {
        ...
        calendarSwitch.prototype = {
            init: function() {
                ...
                var html = "<div class='headerWrapper'><div class='headerTip'>1、请选择入住离店日期</div><div class='comfire'>完成</div><div class='clear'></div></div>";
                html += "<table class='dateZone'><tr><td class='colo'>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td class='colo'>六</td></tr></table>" + "<div class='tbody'></div>";
                ...
                //自定义-房间数量选择
                var addhtml = '<div style="height: 60px;clear: both"></div>';
                addhtml += '<div style="position: fixed; background: #fafafa; z-index: 9999; width: 100%; bottom: 0px; left: 0px; height: 50px; line-height: 50px; margin-left: 10px; font-size: 15px;">2、请选择房间数量<select class="rsselect">';
                for(var i = 1; i<= 10; i++){
                    addhtml += '<option value="'+i+'">'+i+'间</option>';
                }
                addhtml += '</select></div>';
                $(me.sections).append(addhtml);
