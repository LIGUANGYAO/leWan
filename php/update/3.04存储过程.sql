...
-- ----------------------------
-- View structure for `view_forjiesuanciyue`
-- ----------------------------
DROP VIEW IF EXISTS `view_forjiesuanciyue`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `view_forjiesuanciyue` AS select `jay_account_commission201902`.`user_id` AS `user_id`,sum(`jay_account_commission201902`.`record_amount`) AS `record_amount`,'2019年02月' AS `tag` from `jay_account_commission201902` where ((`jay_account_commission201902`.`record_action` in (606,604,607,609,610)) and (`jay_account_commission201902`.`record_status` = 1)) group by `jay_account_commission201902`.`user_id` order by NULL ;

-- ----------------------------
-- View structure for `view_forjiesuanoneandone`
-- ----------------------------
DROP VIEW IF EXISTS `view_forjiesuanoneandone`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `view_forjiesuanoneandone` AS select `r`.`order_id` AS `order_id`,`r`.`user_id` AS `user_id`,sum(`r`.`record_amount`) AS `record_amount`,`r`.`record_attach` AS `record_attach` from `view_tempjiesuanoneandone` `r` group by `r`.`user_id`,`r`.`order_id` ;

-- ----------------------------
-- View structure for `view_orderfinance`
-- ----------------------------
DROP VIEW IF EXISTS `view_orderfinance`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `view_orderfinance` AS select `o`.`order_id` AS `order_id`,`o`.`order_no` AS `order_no`,`o`.`order_transaction` AS `order_transaction`,`o`.`order_fullname` AS `order_fullname`,`o`.`order_mobile` AS `order_mobile`,`o`.`order_totalfee` AS `order_totalfee`,`o`.`order_isexpress` AS `order_isexpress`,`a`.`cash` AS `cash`,`a`.`coupon` AS `coupon`,`a`.`payamount` AS `payamount`,`m`.`merchant_name` AS `merchant_name`,`o`.`order_payment` AS `order_payment`,`o`.`order_addtime` AS `order_addtime`,`o`.`order_paytime` AS `order_paytime`,`p`.`product_id` AS `product_id`,`p`.`product_name` AS `product_name`,`p`.`product_property` AS `product_property`,`p`.`num` AS `num`,`p`.`price` AS `price`,`p`.`settle` AS `settle`,`p`.`totalmoney` AS `totalmoney`,`p`.`totalsettle` AS `totalsettle`,`p`.`commis_free` AS `commis_free`,ifnull(`p`.`commis_first`,0) AS `commis_first`,ifnull(`p`.`commis_second`,0) AS `commis_second`,ifnull(`p`.`commis_operations`,0) AS `commis_operations`,ifnull(`p`.`commis_operations_child`,0) AS `commis_operations_child`,ifnull(`p`.`commis_playerhost_child`,0) AS `commis_playerhost_child`,ifnull(`p`.`commis_playerhost_zhishu`,0) AS `commis_playerhost_zhishu`,`p1`.`catefinance_id` AS `catefinance_id`,`o`.`order_refundstatus` AS `order_refundstatus`,`rf`.`refund_uptime` AS `refund_uptime`,`rf`.`refund_type` AS `refund_type` from (((((`jay_order` `o` left join `jay_order_affiliated` `a` on((`a`.`order_id` = `o`.`order_id`))) left join `jay_order_product` `p` on((`p`.`order_id` = `o`.`order_id`))) left join `jay_order_refund` `rf` on((`rf`.`order_id` = `o`.`order_id`))) left join `jay_product` `p1` on((`p1`.`product_id` = `p`.`product_id`))) left join `jay_merchant` `m` on((`m`.`merchant_id` = `o`.`merchant_id`))) where (`o`.`order_status` > 1) order by `o`.`order_id` desc ;

-- ----------------------------
-- View structure for `view_ordermerchantdaodianfinance`
-- ----------------------------
DROP VIEW IF EXISTS `view_ordermerchantdaodianfinance`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `view_ordermerchantdaodianfinance` AS select `cp`.`consumption_id` AS `consumption_id`,`cp`.`addtime` AS `comsumeaddtime`,`cp`.`remark` AS `remark`,`r`.`reservation_payment` AS `reservation_payment`,`r`.`reservation_addprice` AS `reservation_addprice`,`o`.`order_id` AS `order_id`,`o`.`order_no` AS `order_no`,`c`.`consume_code` AS `consume_code`,`o`.`order_fullname` AS `order_fullname`,`o`.`order_mobile` AS `order_mobile`,`o`.`order_reservation` AS `order_reservation`,`o`.`order_totalfee` AS `order_totalfee`,`o`.`order_payment` AS `order_payment`,`o`.`order_paytime` AS `order_paytime`,`o`.`order_addtime` AS `order_addtime`,`a`.`cash` AS `cash`,`a`.`coupon` AS `coupon`,`a`.`payamount` AS `payamount`,`op`.`product_id` AS `product_id`,`op`.`product_name` AS `product_name`,`op`.`product_property` AS `product_property`,`op`.`num` AS `num`,`op`.`price` AS `price`,`op`.`settle` AS `settle`,`op`.`totalmoney` AS `totalmoney`,`op`.`totalsettle` AS `totalsettle`,ifnull(`op`.`commis_free`,0) AS `commis_free`,ifnull(`op`.`commis_first`,0) AS `commis_first`,ifnull(`op`.`commis_second`,0) AS `commis_second`,ifnull(`op`.`commis_operations`,0) AS `commis_operations`,ifnull(`op`.`commis_operations_child`,0) AS `commis_operations_child`,ifnull(`op`.`commis_playerhost_child`,0) AS `commis_playerhost_child`,ifnull(`op`.`commis_playerhost_zhishu`,0) AS `commis_playerhost_zhishu`,`p`.`catefinance_id` AS `catefinance_id`,`m`.`merchant_name` AS `merchant_name`,`o`.`order_refundstatus` AS `order_refundstatus`,`rf`.`refund_uptime` AS `refund_uptime` from ((((((((`jay_order_consumption` `cp` left join `jay_order_user_reservation` `r` on((`r`.`reservation_id` = `cp`.`reservation_id`))) left join `jay_order_consume_code` `c` on((`c`.`consume_code_id` = `cp`.`consume_code_id`))) left join `jay_order` `o` on((`o`.`order_id` = `cp`.`order_id`))) left join `jay_order_refund` `rf` on((`rf`.`order_id` = `o`.`order_id`))) left join `jay_order_affiliated` `a` on((`a`.`order_id` = `cp`.`order_id`))) left join `jay_order_product` `op` on((`op`.`order_id` = `cp`.`order_id`))) left join `jay_product` `p` on((`p`.`product_id` = `op`.`product_id`))) left join `jay_merchant` `m` on((`m`.`merchant_id` = `o`.`merchant_id`))) ;
...
-- ----------------------------
-- View structure for `view_tempjiesuanoneandone`
-- ----------------------------
DROP VIEW IF EXISTS `view_tempjiesuanoneandone`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `view_tempjiesuanoneandone` AS select `jay_account_commission201902`.`record_id` AS `record_id`,`jay_account_commission201902`.`user_id` AS `user_id`,`jay_account_commission201902`.`order_id` AS `order_id`,`jay_account_commission201902`.`record_currency` AS `record_currency`,`jay_account_commission201902`.`record_action` AS `record_action`,`jay_account_commission201902`.`record_amount` AS `record_amount`,`jay_account_commission201902`.`record_balance` AS `record_balance`,`jay_account_commission201902`.`record_addtime` AS `record_addtime`,`jay_account_commission201902`.`record_attach` AS `record_attach`,`jay_account_commission201902`.`record_remark` AS `record_remark`,`jay_account_commission201902`.`record_status` AS `record_status` from `jay_account_commission201902` where ((`jay_account_commission201902`.`record_status` = 1) and (`jay_account_commission201902`.`record_action` in (601,602,603))) union all select `jay_account_commission201903`.`record_id` AS `record_id`,`jay_account_commission201903`.`user_id` AS `user_id`,`jay_account_commission201903`.`order_id` AS `order_id`,`jay_account_commission201903`.`record_currency` AS `record_currency`,`jay_account_commission201903`.`record_action` AS `record_action`,`jay_account_commission201903`.`record_amount` AS `record_amount`,`jay_account_commission201903`.`record_balance` AS `record_balance`,`jay_account_commission201903`.`record_addtime` AS `record_addtime`,`jay_account_commission201903`.`record_attach` AS `record_attach`,`jay_account_commission201903`.`record_remark` AS `record_remark`,`jay_account_commission201903`.`record_status` AS `record_status` from `jay_account_commission201903` where ((`jay_account_commission201903`.`record_status` = 1) and (`jay_account_commission201903`.`record_action` in (601,602,603))) ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `deduction_Fee`(in userId int, in amount DECIMAL(10,2), in taxfee_fuwu DECIMAL(10,2), in taxfee_geren DECIMAL(10,2), in taxfee_pingtai DECIMAL(10,2), in recordId int, out error int)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `jiesuan_commissionToCashAll`(in userId int, in amount DECIMAL(10,2), in attach VARCHAR(800), in taxfee_fuwu DECIMAL(10,2),in taxfee_geren DECIMAL(10,2),in taxfee_pingtai DECIMAL(10,2), out error int)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `jiesuan_commissionToCashAllYIjian`(out error int)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `jiesuan_commissionToCashOneAndOne`(in userId int, in orderId int, in amount DECIMAL(10,2), in attach VARCHAR(800), in taxfee_fuwu DECIMAL(10,2),in taxfee_geren DECIMAL(10,2),in taxfee_pingtai DECIMAL(10,2), out error int)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `lewan_exchangeUserPath`(in fromUserId int, in toUserId int, OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `lewan_makeuserteamdata`(in userLevel int, OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `lewan_order_income`(in orderId int, OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `lewan_orderyuyue_income`(in reservationOrderId int, OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `lewan_user_performance`(in orderId int, OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `merchant_daodian_income`(in consumeCodeId int, in merchantId int, OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `merchant_kuaidi_income`(in orderId int, OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `Timer_order_autodelivery`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_cancelOrder`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_ciriJiesuan`(out error int)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `Timertask_lewan_pt_income`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_recordtable`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_upAccountMonth`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `Timertask_user_tongji`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_userFinance`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_usertop300lashmonth`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_usertop300month`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `TimerTask_zidongshouqing`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `tj_lastMonthAccount`(OUT error INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `update_account`(in `userId` int, in `currenty` varchar(20), in `amount` DECIMAL(10,2), in orderId int, in action int, in attach varchar(800), in remark varchar(50),  out error int, out recordId int)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` FUNCTION `addPerformance`(userId int, tagstr int, amount DECIMAL(10,2), xiaoliang DECIMAL(10,2), shuncha DECIMAL(10,2), maoli DECIMAL(10,2)) RETURNS int(4)
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` FUNCTION `addProductPerformance`(productId int, tagstr int, amount DECIMAL(10,2), xiaoliang DECIMAL(10,2), shuncha DECIMAL(10,2), maoli DECIMAL(10,2)) RETURNS int(4)
    SQL SECURITY INVOKER
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` FUNCTION `addTimerlog`(pname VARCHAR(50), errorstatis int, stime int) RETURNS int(4)
    SQL SECURITY INVOKER
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` FUNCTION `fuc_manage_finance`(tagstr int, amount DECIMAL(10,2)) RETURNS int(4)
    SQL SECURITY INVOKER
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` FUNCTION `fuc_merchant_account`(merchantId int, tagstr int, amount DECIMAL(10,2)) RETURNS int(4)
    SQL SECURITY INVOKER
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `lewan_zidongshouqing` ON SCHEDULE EVERY 1 HOUR STARTS '2019-02-18 11:30:07' ON COMPLETION PRESERVE ENABLE DO call TimerTask_zidongshouqing(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_autodelivery` ON SCHEDULE EVERY 2 HOUR STARTS '2018-11-23 15:38:25' ON COMPLETION PRESERVE ENABLE DO call Timer_order_autodelivery(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_automakeuserdata` ON SCHEDULE EVERY 4 HOUR STARTS '2018-12-25 09:32:49' ON COMPLETION PRESERVE ENABLE DO out_label: begin
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_cancelorder` ON SCHEDULE EVERY 5 MINUTE STARTS '2018-10-10 00:00:00' ON COMPLETION PRESERVE ENABLE DO call TimerTask_cancelOrder(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_cirijiesuan` ON SCHEDULE EVERY 1 DAY STARTS '2018-09-20 00:01:00' ON COMPLETION PRESERVE ENABLE DO call TimerTask_ciriJiesuan(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_createtable` ON SCHEDULE EVERY 1 MONTH STARTS '2018-09-01 00:00:01' ON COMPLETION PRESERVE ENABLE DO call TimerTask_recordtable(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_manage_income` ON SCHEDULE EVERY 1 DAY STARTS '2018-11-01 00:03:00' ON COMPLETION PRESERVE ENABLE DO call Timertask_lewan_pt_income(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_user300lastmonth` ON SCHEDULE EVERY 1 MONTH STARTS '2019-02-01 06:00:00' ON COMPLETION PRESERVE ENABLE DO call TimerTask_usertop300lashmonth(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_user300month` ON SCHEDULE EVERY 1 DAY STARTS '2019-02-01 06:30:00' ON COMPLETION PRESERVE ENABLE DO call TimerTask_usertop300month(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_userfinance` ON SCHEDULE EVERY 1 DAY STARTS '2018-09-10 05:00:00' ON COMPLETION PRESERVE ENABLE DO call TimerTask_userFinance(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_userlashmonthaccount` ON SCHEDULE EVERY 1 MONTH STARTS '2018-09-01 02:00:01' ON COMPLETION PRESERVE ENABLE DO call TimerTask_upAccountMonth(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_usertongji` ON SCHEDULE EVERY 1 DAY STARTS '2018-12-02 00:02:00' ON COMPLETION PRESERVE ENABLE DO call Timertask_user_tongji(@error)
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_yuyueexpired` ON SCHEDULE EVERY 1 DAY STARTS '2018-11-09 17:45:42' ON COMPLETION PRESERVE ENABLE DO update jay_order_user_reservation r,jay_order_consume_code c set r.reservation_status = 3, c.`status`=3
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_zidongshouqing2` ON SCHEDULE EVERY 1 HOUR STARTS '2019-02-18 15:55:14' ON COMPLETION PRESERVE ENABLE DO update jay_product set sold_out = 1,sold_out_time=UNIX_TIMESTAMP() where product_timelimit=1 and product_endtime<UNIX_TIMESTAMP() and sold_out=0
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_zidongxiajia` ON SCHEDULE EVERY 5 MINUTE STARTS '2018-12-12 21:00:00' ON COMPLETION PRESERVE ENABLE DO update jay_product set sold_out = 1, sold_out_time=UNIX_TIMESTAMP() where product_timelimit=1 and sold_out=0 and product_endtime < UNIX_TIMESTAMP() and product_isexpress=1
;;
DELIMITER ;
