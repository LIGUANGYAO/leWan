...
BEGIN
    ...
    SET @v_sql = CONCAT(@v_sql, " select user_id, sum(finance_xrmd+finance_first+finance_second+finance_operations+finance_operationchilds+finance_playerhost+finance_playerhostzhishu+finance_reward+finance_rewardback+finance_reward10+finance_rewardxrzm) commission, '",@lastmoth_tag,"', UNIX_TIMESTAMP() from jay_account_finance  ");
...
BEGIN
    ...
    SET @v_sql = CONCAT(@v_sql, " select user_id, sum(finance_xrmd+finance_first+finance_second+finance_operations+finance_operationchilds+finance_playerhost+finance_playerhostzhishu+finance_reward+finance_rewardback+finance_reward10+finance_rewardxrzm) commission, '",@month_tag,"', UNIX_TIMESTAMP() from jay_account_finance  ");
