...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_manage_income` ON SCHEDULE EVERY 1 DAY STARTS '2018-11-01 03:00:00'
...
CREATE VIEW `view_orderFinance` AS
  SELECT `o`.`order_id`,`o`.`order_no`,`o`.`order_transaction`,`o`.`order_fullname`,`o`.`order_mobile`,`o`.`order_totalfee`,`o`.`order_isexpress`,
    ...
    `p`.`userid_first`,`p`.`userid_second`,`p`.`userid_operations`,`p`.`userid_operations_child`,`p`.`userid_playerhost_child`,
    `p1`.`catefinance_id`
  FROM jay_order o
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_cancelorder`
...
CREATE DEFINER=`root`@`%` EVENT `timer_zidongxiajia`
  ON SCHEDULE EVERY 1 DAY STARTS '2018-11-09 17:18:38'
  ON COMPLETION NOT PRESERVE ENABLE
DO
  update jay_product set product_status = 0 where product_timelimit=1 and product_endtime < UNIX_TIMESTAMP()-172800;
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_yuyueexpired`
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_autodelivery`
