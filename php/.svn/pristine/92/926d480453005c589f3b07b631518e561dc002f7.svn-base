...
class Uploader
{
    ...
    private function upFile()
    {
        ...
        if(in_array($this->getFileExt(), $this->config["allowFiles"])){
            ...
            $url =  $host.'/api/upload/ossUpload/';
            $data['fileName'] =  'uplaod/g_details/' . date("Ymd") . '/' . sha1(date('YmdHis', time()) . uniqid()) . $format;
...
/**
    ...
    private function upBase64()
    {
        ...
        if (!(file_put_contents($this->filePath, $img) && file_exists($this->filePath))) { //移动失败
            ...
            $new_file_name = sha1($this->filePath) .'.'.$this->getFileExt();
            $host =  $_SERVER['HTTP_HOST']?$_SERVER['HTTP_HOST']: $_SERVER['SERVER_NAME'];
            $url =  $host.'/api/upload/ossUpload/';
