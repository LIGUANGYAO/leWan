...
class ProductController extends AdminBaseController {
    ...
    public function inview(){
        if(Request::instance()->isPost()){
            ...
            if($product){
                $res = Db::name('product')->where('product_id', $id)->update(['product_reviewstatus'=>$status, 'product_remark'=>$reason, 'product_uptime'=>time()]);
    ...
    private function loadList($map){
        ...
        if ($city_id){
            ...
            $this->assign('city_id', $provence_id);
        }
        if($starttime){
    ...
    public function edit(){
        if(Request::instance()->isPost()){
            ...
            if($res1 !== false){
                ...
                if($data['product_toplevel'] < 1){
                    ...
                    ->where(['product_id'=>$product_id,'provence_code'=>$merchant['merchant_pcode']])
                    ->delete();
                }
                Db::commit();
        ...
        if( $product['purchase_id']){
            ...
            $product['purchase'] = Db::name('user')->field('user_id,nickname,mobile')->where($umap)->find();
        }
        if( $product['duty_id']){
            $umap['staff_id'] = $product['duty_id'];
            $product['duty'] = Db::name('staff')->field('staff_id,staff_name,staff_tel')->where($umap)->find();
        }
    ...
    public function editprice(){
        if(Request::instance()->isPost()){
            ...
            Db::name("product")->where("product_id","=",$prid)->update($Data);
    ...
    public function productCategoryEdit(){
        if (Request::instance()->isGet()){ //显示
            ...
            if($type== 'add'){
                ...
                }else if($type == 'del'){ //删除 (暂时不考虑多级分类)
                    $res = Db::name('product_category')->delete($id);
    ...
    public function sendProductToCity(){
        if(Request::instance()->isPost()){
            ...
            if(!in_array($data['merchant_city'],$data['city'])){
                array_push($data['city'],$data['merchant_city']);//商家默认城市必须推送
            }

            if(!$data['product_id'] || empty($data['city'])){
                $this->error('推送错误，请重试');
            }
            $product = Db::name('product p')
                ->join('merchant m', 'm.merchant_id = p.merchant_id', 'left')
                ->field('p.product_toplevel,m.merchant_pcode')->where(['product_id'=>$data['product_id']])->find();
            if(empty($product)){
                $this->error('商品不存在');
            }
            Db::startTrans();
            $rs = Db::name('product')->where(['product_id'=>$data['product_id']])->update(['push_city_codes'=>implode(',',$data['city'])]);
            if($rs!==false){
                foreach ($data['city'] as $val){
                    if($val){
                        if(!ProductModel::pushProductToCity($data['product_id'],$val)){
                            Db::rollback();
            ...
            $list = Db::name('home_product_city')->field('city_code')->where(['product_id'=>$product_id])->select();
            $city_code=[];
            if($list){
                foreach ($list as $val){
                    if(!empty($val)){
                ...
                $merchant_city = Db::name('merchant')->where(['merchant_id'=>$merchant_id])->value('merchant_ccode');
            }
    ...
    public function refreshTime(){
        ...
        if($product_id){
            if(false !== Db::name('product')->where(['product_id'=>$product_id])->update(['product_uptime'=>time()])){
                return array('code'=>200);
            }
        }
    ...
    public function homeproductrank(){
        $type = $this->request->get('type','city');
        if($type == 'city'){
            ...
            $list = Db::name('home_product_top_provence p')
                ->field('p.*,r.name')
                ->join('region r','r.id=p.provence_code')
                ->group('p.provence_code')
                ->select();
        }
        $this->assign('list', $list);
        $this->assign('type',  $this->request->get('type','city'));
        ...
        if(Request::instance()->isPost()) {
            ...
            foreach ($result as $k => $v) {
                if ($v) {
                    ...
                    $merchant = Db::name('merchant')->field('parent_id,merchant_id')->where(['merchant_id' => $v['merchant_id']])->find();
                    ...
                    if (Db::name('product_kuaidikucun')->where($map)->count()) {
                        if (false === Db::name('product_kuaidikucun')->where($map)->update($v)) {
                            $this->error("提交失败");
                        }
                    } else {
                        $v['addtime'] = time();
                        if (!Db::name('product_kuaidikucun')->insertGetId($v)) {
                            $this->error("提交失败");
                        }
                    }
                }
