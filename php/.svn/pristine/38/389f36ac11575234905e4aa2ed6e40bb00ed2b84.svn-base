...
class UserController extends ApiBaseController{
    ...
    public function UserPersonal(){
        try{
            ...
            $UserData  = UserModel::UserDataFind(array("u.token"=>array("eq",$Token)),"u.token,u.recode,u.nickname,u.username as user_name,u.avatar,u.userthumb,u.level,u.auth,u.freesheet");
    ...
    public function UserWallet(){
        try{
           ...
           // $jsamount = Db::name('account_cash'.Tag::getMonth())->where(['user_id'=>$Uid, 'record_addtime'=>['between',$jrtime.','.($jrtime+86400)], 'record_action'=>['in', '802,803']])->sum('record_amount');
            ...
            $Sumup = $ufinance['finance_withdraw'] + abs($txamount);
            $UserReward["income"]     = $Acc["account_commission_income"]>0?$Acc["account_commission_income"]:0;
            $UserReward["putforward"] = $Acc["account_cash_balance"]>0?$Acc["account_cash_balance"]:0;
            $UserReward["pending"]    = $Acc["account_commission_balance"]>0?$Acc["account_commission_balance"]:0;
            $UserReward["grandtotal"] = abs($Acc["account_commission_expenditure"])>0?abs($Acc['account_commission_expenditure']):0;
            $UserReward["sumup"]      = $Sumup>0?$Sumup:0;
