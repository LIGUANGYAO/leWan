...
class UsercommunityleaderController extends ApiBaseController{
    ...
    public function top300(){
        $user = getUserByToken();
        //本月佣金金额
        $byamount = Db::name('account')->where(['user_id'=>$user['user_id'],'account_tag'=>date('Ym')])->find();
        $data['commission_benyue'] = sprintf('%.2f', $byamount['commission']);
        //本月排名
        ...
        if($by){
            ...
            $data['chaju'] = '相差'.ceil(($dym['commission']-$by['commission'])/$dym['commission']*100).'%';
        }else{
