...
class YuyueController extends ApiBaseController
{
    ...
    public function ConsumeBookedConfirm(){
        try{
          ...
          //  $Name    = input("post.fullname","","htmlspecialchars,strip_tags");
...
//            parent::Tpl_Empty($Name,"请输入联系人姓名",2);
//            parent::Tpl_FullSpace($Name,"请输入联系人姓名");
            parent::Tpl_Phone($Phone,"请输入正确的手机号",2);
            parent::Tpl_Empty($Code,"请输入电子码",2);
            parent::Tpl_FullSpace($Code,"请输入电子码");

//            $Condition["o.order_fullname"]    = $Name;
            $Condition["o.order_mobile"]      = $Phone;
            $Condition["o.order_status"]      = 2;
            $Condition["cc.consume_code"]     = $Code;
           // $Condition[]                   = array("exp","r.reservation_status = 0 or r.reservation_status = 4");

            $Order = OrderModel::OrderConfirm($Condition);

            if (!$Order){
                $this->returnApiData("请填写正确的订单/联系电话/消费码", 400);
            }else{
    ...
    public function ConsumeBookedProductDet(){
        try{
            ...
            $OrderFind = OrderModel::OrderFind($OrderCondition,"o.order_id,p.product_endusetime");
