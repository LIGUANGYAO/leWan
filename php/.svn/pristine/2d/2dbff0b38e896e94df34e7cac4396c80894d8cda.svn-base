...
class BranchorderModel extends BaseModel{
    ...
    public static function TableName(){
        ...
        $DatabaseSub   = Config::get('database_sub');
        $DatabaseSub   = $DatabaseSub["database"].".".$DatabaseSub["prefix"];
        $DatabaseDistr = Config::get('database_distr');
        $DatabaseDistr = $DatabaseDistr["database"].".".$DatabaseDistr["prefix"];

        return array($Database,$DatabaseSub,$DatabaseDistr);
    }
    ...
    public static function OrderList($Condition = array(),$Psize = 1,$PageSize = 50){
        $Field     = "o.*,au.sub_name,fa.company,p.num,p.price,pr.distributiontag,pp.product_property,u.mobile,u.nickname,m.merchant_name,
        ...
        $Count     = Db::table($Table[0]."order")
                        ->alias("o")
                        ->field($Field)
                        ->join($Table[1]."admin_user au","au.id=o.fgs_admin_user_id","left")
                        ->join($Table[2]."admin_user fa","fa.id=o.fx_admin_user_id","left")
    ...
    public static function OrderFind($OrderId){
        $Field  = "o.*,au.sub_name,fa.company,fa.concat,fa.mobile as fxmobile,af.cash,af.points,af.commission,af.coupon,
        af.payamount,u.nickname,u.mobile,a.contact,a.mobile as amobile,a.ssq,a.address,r.refund_uptime,r.refund_status";
        $Gfield = "op.*,p.distributiontag,p.distributiontag,m.merchant_name";
        $Cfield = "cc.consume_code_id,cc.consume_code,cc.status,cc.addtime,cc.uptime,rd.week,rd.calendar,rp.addprice,
        rp.totalnum,rp.usenum";
