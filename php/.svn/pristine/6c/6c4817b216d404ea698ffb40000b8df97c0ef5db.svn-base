...
class AdminBaseController extends BaseController {
     ...
     * 获取省份列表
     */
    protected function getProvenceList() {
        return Db::name('provence')->where('1=1')->order('id asc')->select();
    }

    /**
     * 获取省份列表
     */
    protected function getCityList($pid) {
        return Db::name('city')->where(['pcode'=>$pid])->order('id asc')->select();
    }

    /**
     * 获取省份列表
     */
    protected function getAreaList($cid) {
        return Db::name('area')->where(['ccode'=>$cid])->order('id asc')->select();
    }
