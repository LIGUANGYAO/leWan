...
class BaseController extends Controller
{
    ...
    function Tpl_Abnormal($Error){
        if(!empty($Error)){
            ...
            //要写入的内容
            $AddLogStr = "\r\n".date('Y-m-d H:i:s')."记录了一条数据库错误信息\r\n";//记录日志开头的标识
            //打开资源并将光标设置为末尾
            $Fp = fopen($Txt,"a+");//判断文件是否存在,不存在添加文件
            //写入内容
            fwrite($Fp,$AddLogStr.$Error);
            //关闭资源
            fclose($Fp);
        }
        $Error        = explode(':', $Error);
        ...
        if(!empty($Error[0])){
            if(in_array($Error[0],$AataBase)){
                CommonController::Messagereturn(CommonController::ResponseFailed,"服务器响应失败");
    ...
    function Tpl_NotGtTime($Time,$News,$Visit = 1){
        ...
        if ($Time > $CurrentTime) {
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,$News);
            }
        }
    }
    ...
    function Tpl_Time($Time = null,$Data,$Day = 1){
        if(!empty($Time)){
            ...
            switch ($Data){
                ...
                case 5:
                    return  strtotime("+$Day day", strtotime($Time)); //时间2017-09-06 这样的格式转换为时间戳格式并加天数默认加一天
                case 6:
                    return  strtotime("-$Day day", strtotime($Time)); //时间2017-09-06 这样的格式转换为时间戳格式并减天数，默认减一天
                case 7:
                    return  strtotime("+$Day hours", strtotime($Time)); //时间2017-09-06 这样的格式转换为时间戳格式并加小时,默认一小时
                case 8:
                    return  strtotime("-$Day hours", strtotime($Time)); //时间2017-09-06 这样的格式转换为时间戳格式并减小时,默认一小时
                case 9:
                    return  strtotime("+$Day minutes", strtotime($Time)); //时间2017-09-06 这样的格式转换为时间戳格式并加分钟,默认一分钟
                case 10:
                    return  strtotime("-$Day minutes", strtotime($Time)); //时间2017-09-06 这样的格式转换为时间戳格式并减分钟,默认一分钟
                default:
                    return "";
            }
        }else{
    ...
    function Tpl_TimeContrast($StartTime,$EndTime,$Visit = 1){
        ...
        if ($StartTime > $EndTime) {
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"开始时间不能大于结束时间");
            }
        }else if($StartTime == $EndTime){
            if($Visit == 1){
                $this->commonReturn("开始时间和结束时间不能相等", 0);
            }else if($Visit == 2){
                CommonController::Messagereturn(CommonController::ParameterEror,"开始时间和结束时间不能相等");
            }
        }else if ($StartTime > $Time || $EndTime > $Time) {
            if($Visit == 1){
                $this->commonReturn("开始时间和结束时间不能大于当前时间", 0);
            }else if($Visit == 2){
                CommonController::Messagereturn(CommonController::ParameterEror,"开始时间和结束时间不能大于当前时间");
            }
        }
    }
            ...
            if(!func_encryption($Password)){
                if($Data == 1){
                    if($Visit == 1){
                        ...
                        CommonController::Messagereturn(CommonController::ParameterEror,"密码只能是a-zA-Z0-9,特殊符号,其中的两种组成,不能有空格不能小于6位大于16位");
                    }
                }else if($Data == 2){
        ...
        if($NewPwd != $ConfirmPwd){
            if($Data == 1){
                if($Visit == 1){
                    ...
                    CommonController::Messagereturn(CommonController::ParameterEror,"两次密码不一致请重新输入密码");
                }
            }else if($Data == 2){
    ...
    function Tpl_Percentage($Data,$Visit = 1){
        if(!Func_Percentage($Data)){
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"比例只能是整数或者小数,小数位数最大为2位,不能为空,不能大于100小于0.00");
            }
        }
    }
     ...
     * 判断手机格式是否正确
     */
    function Tpl_Phone($Name,$Visit = 1,$News = "手机格式错误,请输入正确的手机号码"){
        if(!pregPhone($Name)){
            if($Visit == 1){
    ...
    function Tpl_PaymentPwd($Name,$Visit = 1){
        if(!Func_Payment($Name)){
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"支付密码格式错误,只能是数字必须是6位");
            }
        }
    }
    ...
    function Tpl_Email($Name,$Visit = 1){
        if(!Func_Email($Name)){
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"邮箱错误,必须含有@ .,如123@qq.com");
            }
        }
    }
    ...
    function Tpl_IdCard($Name,$Visit = 1){
        if(!Func_IdCard($Name)){
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"身份证号码错误只能是15位和18位数字或者18位结尾为Xx结尾的格式");
            }
        }
    }
    ...
    function Tpl_DateCard($Name,$Visit = 1){
        if(!ValiDateCard($Name)){
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"银行卡格式错误");
            }
        }
    }
    ...
    function Tpl_Url($Url,$Visit = 1){
        if(!Func_url($Url)){
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"请输入正确的网址格式地址,比如https://www.baidu.com.");
            }
        }
    }
    ...
    function Tpl_Lengths($Name,$News,$Min=2,$Max=10,$Visit=1){
        if (mb_strlen($Name,'utf-8') < $Min || mb_strlen($Name,'utf-8') > $Max) {
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"位数超过限制");
            }
        }
    }
    /**
    ...
    function Tpl_TimeShutup($StartTime,$EndTime,$Visit=1){
        ...
        if ($StartTime > $EndTime) {
            ...
            if($Visit == 1){
                ...
                CommonController::Messagereturn(CommonController::ParameterEror,"开始时间不能小于当前时间");
            }
        }
    }
