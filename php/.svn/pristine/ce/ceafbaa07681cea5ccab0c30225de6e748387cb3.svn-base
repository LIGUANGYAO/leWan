...
-- ----------------------------
-- [存储过程：次日结算佣金到现金,一个交易一个交易的结算-内部调用]
...
-- ----------------------------
-- [存储过程：次月累计佣金到现金，一个人只有1条累计结算-内部调用]
-- 传入每个用户 统计金额，具体待定
...
BEGIN
    ...
    SET @v_sql = CONCAT(@v_sql, ' WHERE record_action in (606,604,607) and record_status = 1 ');
...
DELIMITER ;
-- ----------------------------
-- [自动统计上月收支]Procedure structure for `tj_lastMonthAccount`
-- ----------------------------
...
-- ----------------------------
-- [取消订单存储过程]Procedure structure for `TimerTask_cancelOrder`
-- ----------------------------
    ...
    out_label: BEGIN
      ...
      repeat
        ...
        IF NOT done THEN
          ...
          #更新商品
          SET @v_sql = CONCAT('UPDATE jay_product SET sold_out = 0,sold_out_time=0 where product_id = ', productId);
          PREPARE statement FROM @v_sql;
          EXECUTE statement;
          DEALLOCATE PREPARE statement;
...
-- ----------------------------
-- 商家核销结算
-- [到店订单完成结算给商家]Procedure structure for `merchant_daodian_income`
-- ----------------------------
...
-- ----------------------------
-- 确认收货结算
-- [快递订单完成结算给商家]Procedure structure for `merchant_kuaidi_income`
-- ----------------------------
...
-- ----------------------------
-- 订单付款统计利润1，付款后调用
-- [平台利润表]Procedure structure for `lewan_order_income`
-- ----------------------------
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `lewan_order_income`(in orderId int, OUT error INT)
  BEGIN
    ...
    into @totalfee,@num,@totalsettle,@coupon,@payamount,@commis_first,@commis_second,@commis_operations,@commis_operations_child,@commis_playerhost_child,@isreturnall from jay_order o
      left join jay_order_product op on op.order_id = o.order_id
    ...
    out_label: BEGIN
      ...
      IF @totalfee > 0 THEN
        ...
        #5.更新实时统计
        SET @totalcom = @commis_first+@commis_second+@commis_operations+@commis_operations_child+@commis_playerhost_child+@returnmoney;
...
-- ----------------------------
-- 预约加价统计2，付款后调用
-- [平台利润表]Procedure structure for `lewan_orderyuyue_income`
-- ----------------------------
...
-- ----------------------------
-- (平台统计其他项)-昨日
-- [平台利润表]Procedure structure for `Timertask_lewan_pt_income`
-- ----------------------------
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Timertask_lewan_pt_income`(OUT error INT)
  BEGIN
    ...
    SET @v_sql = CONCAT('select ifnull(sum(record_amount),0) into @total_yijiesuan from jay_account_cash',@yesterdaymonth_tag,' where record_action in (802,803);');
    PREPARE statement FROM @v_sql;
    EXECUTE statement;
    DEALLOCATE PREPARE statement;
    #2.昨日总提现成功金额
    ...
    SET @v_sql = CONCAT('select ifnull(abs(sum(record_amount)),0) into @total_shouxufei from jay_account_cash',@yesterdaymonth_tag,' where record_action in (854,855,856,857);');
    PREPARE statement FROM @v_sql;
    EXECUTE statement;
    DEALLOCATE PREPARE statement;
    #4.商家结算总金额
    SET @total_shangjiajiesuan = 0;
    SET @v_sql = CONCAT('select ifnull(sum(record_amount),0) into @total_shangjiajiesuan from jay_merchant_account',@yesterdaymonth_tag,' where record_action =952;');
    PREPARE statement FROM @v_sql;
    EXECUTE statement;
    DEALLOCATE PREPARE statement;
    #5.交易用户量
...
-- ----------------------------
-- 订单付款后调用
-- [会员业绩统计]Procedure structure for `lewan_user_performance`
-- ----------------------------
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Timer_order_autodelivery`(OUT error INT)
  BEGIN
    ...
    declare orderId int default 0;
    #获取要统计的用户id
    declare orderList cursor for select order_id from jay_order where order_isexpress=2 and order_reservation = 2 and order_status = 3 and delivery_time + 604800 < UNIX_TIMESTAMP();
    declare continue handler for not found set done = 1;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET error = 1; # 异常错误
    SET error = 0;
...
-- ----------------------------
-- 过期订单视图 `view_forexpireorder`
-- ----------------------------
DROP VIEW IF EXISTS `view_forexpireorder`;
CREATE VIEW `view_forexpireorder` AS
  select o.order_id, ifnull(p.price_id, 0) price_id, ifnull(p.pricecalendar_id,0) pricecalendar_id, p.price_type, p.num, p.product_id from jay_order o
    left join jay_order_product p on p.order_id = o.order_id
  WHERE o.order_status = 1 and o.order_addtime < UNIX_TIMESTAMP()-1800;



-- ----------------------------
-- 订单统计视图 `view_orderFinance`
-- ----------------------------
DROP VIEW IF EXISTS `view_orderFinance`;
CREATE VIEW `view_orderFinance` AS
  SELECT `o`.`order_id`,`o`.`order_no`,`o`.`order_transaction`,`o`.`order_fullname`,`o`.`order_mobile`,`o`.`order_totalfee`,`o`.`order_isexpress`,
    `a`.`cash`,`a`.`coupon`,`a`.`payamount`,`m`.`merchant_name`,`o`.`order_payment`,`o`.`order_addtime`,`o`.`order_paytime`,
    `p`.`product_id`,`p`.`product_name`,`p`.`product_property`,`p`.`num`,`p`.`price`,`p`.`settle`,`p`.`totalmoney`,`p`.`totalsettle`,
    ifnull(p.commis_first,0) commis_first, ifnull(p.commis_second,0) commis_second,ifnull(p.commis_operations,0) commis_operations,ifnull(p.commis_operations_child,0) commis_operations_child,ifnull(p.commis_playerhost_child,0) commis_playerhost_child,
    `p1`.`catefinance_id`, `o`.`order_refundstatus`, rf.refund_uptime
  FROM jay_order o
...
-- ----------------------------
-- 商家到店订单统计视图 `view_orderMerchantDaodianFinance`
-- ----------------------------
DROP VIEW IF EXISTS `view_orderMerchantDaodianFinance`;
CREATE VIEW `view_orderMerchantDaodianFinance` AS
  select cp.consumption_id, cp.addtime as comsumeaddtime, cp.remark,
    r.reservation_payment, r.reservation_addprice,
    o.order_id, o.order_no, c.consume_code, o.order_fullname, o.order_mobile, o.order_reservation, o.order_totalfee, o.order_payment, o.order_paytime,
    o.order_addtime, a.cash, a.coupon, a.payamount, op.product_id,
    op.product_name, op.product_property, op.num, op.price, op.settle, op.totalmoney, op.totalsettle,
    ifnull(op.commis_first,0) commis_first, ifnull(op.commis_second,0) commis_second,ifnull(op.commis_operations,0) commis_operations,ifnull(op.commis_operations_child,0) commis_operations_child,ifnull(op.commis_playerhost_child,0) commis_playerhost_child,
    op.userid_first, op.userid_second, op.userid_operations, op.userid_operations_child, op.userid_playerhost_child,
    p.catefinance_id, m.merchant_name, `o`.`order_refundstatus`, rf.refund_uptime
  from jay_order_consumption cp
    left join jay_order_user_reservation r on r.reservation_id = cp.reservation_id
    left join jay_order_consume_code c on c.consume_code_id = cp.consume_code_id
    left join jay_order o on o.order_id = cp.order_id
    LEFT JOIN `jay_order_refund` `rf` ON `rf`.`order_id`=`o`.`order_id`
...
-- ----------------------------
-- 商家快递订单统计视图 `view_orderMerchantKuaidiFinance`
-- ----------------------------
DROP VIEW IF EXISTS `view_orderMerchantKuaidiFinance`;
CREATE VIEW `view_orderMerchantKuaidiFinance` AS
  select o.order_id, o.order_no, o.order_fullname, o.order_mobile, o.order_totalfee, o.order_payment, o.order_paytime,
    o.order_isexpress, o.order_reservation, o.order_addtime, a.cash, a.coupon, a.payamount, op.product_id,
    op.product_name, op.product_property, op.num, op.price, op.settle, op.totalmoney, op.totalsettle,
    ifnull(op.commis_first,0) commis_first, ifnull(op.commis_second,0) commis_second,ifnull(op.commis_operations,0) commis_operations,ifnull(op.commis_operations_child,0) commis_operations_child,ifnull(op.commis_playerhost_child,0) commis_playerhost_child,
    op.userid_first, op.userid_second, op.userid_operations, op.userid_operations_child, op.userid_playerhost_child,
    p.catefinance_id, m.merchant_name
  from jay_order o
    left join jay_order_affiliated a on a.order_id = o.order_id
    left join jay_order_product op on op.order_id = o.order_id
    left join jay_product p on p.product_id = op.product_id
...
-- ----------------------------
-- 函数-记录商品销量利润 `addProductPerformance`
-- ----------------------------
...
-- ----------------------------
-- 函数-记录商家统计 `fuc_merchant_account`
-- ----------------------------
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` EVENT `timer_cirijiesuan`
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` EVENT `timer_userlashmonthaccount`
  ON SCHEDULE EVERY 1 MONTH STARTS '2018-09-01 02:00:01'
  ON COMPLETION NOT PRESERVE ENABLE DO
    call TimerTask_upAccountMonth(@error);
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` EVENT `timer_userfinance`
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` EVENT `timer_cancelorder`
  ON SCHEDULE EVERY 5 MINUTE STARTS '2018-10-10 00:00:00'
  ON COMPLETION NOT PRESERVE ENABLE
  DO
  call TimerTask_cancelOrder(@error);
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` EVENT `timer_zidongxiajia`
  ON SCHEDULE EVERY 5 MINUTE STARTS '2018-12-12 21:00:00' ON COMPLETION PRESERVE ENABLE DO
  update jay_product set sold_out = 1, sold_out_time=UNIX_TIMESTAMP() where product_timelimit=1 and product_endtime < UNIX_TIMESTAMP();
;;
DELIMITER ;
-- ----------------------------
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` EVENT `timer_yuyueexpired`
  ON SCHEDULE EVERY 1 DAY STARTS '2018-11-09 17:45:42'
  ON COMPLETION NOT PRESERVE ENABLE DO
  update jay_order_user_reservation r,jay_order_consume_code c set r.reservation_status = 3, c.`status`=3
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` EVENT `timer_manage_income` ON SCHEDULE EVERY 1 DAY STARTS '2018-11-01 03:00:00'
  ON COMPLETION NOT PRESERVE ENABLE
DO
  call Timertask_lewan_pt_income(@error);
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` EVENT `timer_autodelivery`
  ON SCHEDULE EVERY 2 HOUR STARTS '2018-11-23 15:38:25'
  ON COMPLETION NOT PRESERVE ENABLE
  DO call Timer_order_autodelivery(@error);
;;
DELIMITER ;