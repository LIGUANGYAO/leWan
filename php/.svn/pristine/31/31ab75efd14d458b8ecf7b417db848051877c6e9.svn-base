...
class OrderController extends AdminBaseController {
    ...
    public function OrderList() {
        ...
        $Count["whole"]   = OrderModel::OrderCount($Condition,null,0,$Status);
        $Count["unpaid"]  = OrderModel::OrderCount($Condition,array("o.order_status"=>1),1,$Status);
        $Count["hair"]    = OrderModel::OrderCount($Condition,array("o.order_status"=>2),2,$Status);
        $Count["collect"] = OrderModel::OrderCount($Condition,array("o.order_status"=>3),3,$Status);
        $Count["fulfil"]  = OrderModel::OrderCount($Condition,array("o.order_status"=>4),4,$Status);
        $Count["cancel"]  = OrderModel::OrderCount($Condition,array("o.order_status"=>5),5,$Status);
        $Count["refund"]  = OrderModel::OrderCount($Condition,array("o.order_status"=>6),6,$Status);
        $Count["barter"]  = OrderModel::OrderCount($Condition,array("o.order_status"=>7),7,$Status);
        $Count["expire"]  = OrderModel::OrderCount($Condition,array("o.order_status"=>8),7,$Status);

        $Query = array("title" => $Title,"isexpress" => $Isexpress,"payment"=>$Payment,"type"=>$Type);
    ...
    public function OrderData(){
        ...
        if (Request()->isGet()){
            ...
            foreach ($Status as $Key=>$Val){
                ...
                if ($CodeUp){
                    $Action = "管理员：".Session::get('admin.nickname')."修改订单消费码状态为：".$Val == 1?"恢复":"冻结";
    ...
    private function Transference(){
        $GoodsType   = array("1" => array("css" => "layui-bg-red", "name" => "到店商品"),
                            "2" => array("css" => "layui-bg-green", "name" => "实物商品"),);
        $Reservation = array("0" => array("css" => "layui-bg-gray", "name" => "免预约"),
                             "1" => array("css" => "layui-bg-blue", "name" => "预约制"),
                             "2" => array("css" => "layui-bg-gray", "name" => "免预约"),);
        $PayType     = array("1" => array("css" => "layui-bg-green", "name" => "微信支付"),
                            "2" => array("css" => "layui-bg-blue", "name" => "支付宝支付"),
        ...
        $OrderStatus = array("1" => array("css" => "layui-bg-gray","name" => "待付款"),
                            ...
                            "8" => array("css" => "layui-bg-black","name" => "订单过期"));

        return array($GoodsType,$Reservation,$PayType,$OrderStatus);
    }
