...
class QuestionController extends AdminBaseController {
    ...
    public function index() {
        //设置添加信息按钮
        $this->assign('addbtn', $this->returnAddbtn('发布问题', 'system/question/add', 2));
    ...
    public function add() {
        if (Request::instance()->isGet()) {
            $this->assign('action', url('system/question/add'));
            ...
            $this->log('发布问题：' . $item['title']);
            if ($res !== false) {
                $this->toSuccess('发布成功', 'question/index');
            } else {
                $this->toError('添加失败');
            }
        }
    ...
    public function edit() {
        if (Request::instance()->isGet()) {
            ...
            $this->assign('action', url('system/question/edit'));
            return $this->display('edit', true);
        } else {
            $item['title'] = $this->post('title', '', RegExpression::REQUIRED, '标题');
            $item['section'] = $this->section;
            ...
            $this->log('修改问题：' . $item['title']);
            if ($res !== false) {
                $this->toSuccess('编辑成功', url('question/index'));
            } else {
                $this->toError('编辑失败');
            }
        }
