...
class ProductController extends AdminBaseController {
    ...
    public function editprice(){
        if(Request::instance()->isPost()){
            ...
            $vo['product_id'] = $this->post('product_id', 0);
            $vo['product_property'] = $this->post('property', '');
            $vo['price_market'] = $this->post('market', 0);
            $vo['price_sale'] = $this->post('sale', 0);
            $vo['price_commission'] = $this->post('commission', 0);
            $vo['product_totalnum'] = $this->post('totalnum', 0);
            $vo['product_buynum'] = $this->post('buynum', 0);
            $vo['product_dynamicnum'] = $this->post('dynamicnum', 0);
            $vo['price_addtime'] = time();
            if($vo['price_market'] < $vo['price_sale']){
                $this->error('销售价 不能 高于 市场价');
            }
            if($vo['price_commission'] >= $vo['price_sale']){
                $this->error('佣金 不能 高于 销售价');
            }
            if($vo['product_buynum'] >= $vo['product_totalnum']){
                $this->error('已售数量 不能 大于 库存总数');
            }
            $price = Db::name('product_price')->where(['price_id'=>$price_id])->find();
            if($price){
                Db::name('product_price')->where(['price_id'=>$price_id])->update($vo);
            }else{
                Db::name('product_price')->insert($vo);
            }
            Db::name('product')->where(['product_id'=>$vo['product_id']])->update(['temp_price'=>$vo['price_sale'],'temp_commission'=>$vo['price_commission']]);
            $this->log('编辑商品价格id：'.$vo['product_id']);
            ProductModel::log($vo['product_id'], 1, session('admin.nickname'), session('admin.id'), '编辑价格'.$vo['price_sale']);
            $this->success('修改成功', '', 2);
        }
        $product = Db::name('product')->where(['product_id'=>$this->get('id', 0)])->find();
        $this->assign('obj', $product);
        $price = Db::name('product_price')->where(['product_id'=>$this->get('id', 0)])->find();
    ...
    public function resercanlendar(){
        ...
        $return['canlendar'] = $this->getCalendar($targetmonth, $product['product_startusetime'], $product['product_endusetime'], $product['product_id']);
    ...
    public function reservbatchedit(){
        if(Request::instance()->isPost()){
            ...
            $this->editKucunAndPrice($calendararray,$product_id,$preday);
     ...
     * @param $preday
     */
    private function editKucunAndPrice($calendararray,$product_id,$preday){
        //更新日期数据
        foreach ($calendararray as $day){
            ...
            $where1 = ['product_id'=>$product_id, 'calendar'=>$dd['calendar']];
    ...
    public function deletereserv(){
        ...
        foreach ($calendararray as $k=>$day){
            $reservationday = Db::name('product_reservationday')->where(['product_id'=>$product_id, 'calendar'=>strtotime($day)])->find();
            //删除记录
     ...
     * @param $endtime
     */
    private function getCalendar($month, $starttime, $endtime, $product_id){
