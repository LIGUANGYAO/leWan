...
class RetailadminorderController extends AdminBaseController{
    ...
    public function AdminOrderList(){
        ...
        $DistrId   = $this->get('distrid', 0);//分页默认第一页
        ...
        if ($DistrId){
            $Condition["o.fx_admin_user_id"] = $DistrId;
        }
        if ($Title){
        ...
        $Query = array("distrid"=>$DistrId,"title"=>$Title,"dboss_id"=>$dboss_id,"isexpress" => $Isexpress,
            "reservation"=>$Reservation,"distributiontag"=>$Distributiontag,"payment"=>$Payment);
