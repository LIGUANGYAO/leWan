...
class UsercommunityleaderModel{
    ...
    public static function LeaderlRankingFind($Condition = array()){
        ...
        $Field = "u.avatar,u.nickname,u.ownerstatus,m.id as thismonthranking,
        m.commission,m.addtime,l.id as lastmonthranking";
     ...
     * 肖亚子
     */
    public static function LeaderUserAccount($Condition = array()){
        $Data = Db::name("account")
                ->where($Condition)
                ->group("user_id")
                ->value("sum(account_commission_income) as commission");
