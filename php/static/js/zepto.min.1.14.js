...
var navhomeshow = false;
$('.zcdicon').click(function(){
	if($('#navhome').hasClass('show')){
		navhomeshow = false;
	}else{
		navhomeshow = true;
		$('.shadow').show();
...
function loadcity(pid){
	$.ajax({
	   type: "POST",
	   url: "/index.php?g=Shop&m=RecAddress&a=getcity",
	   data:"pid="+pid,
	   dataType:'json',
	   success: function(res){
		  var html = '<option value="">请选择</option>';
		  for(var i = 0; i< res.length; i++){
			   html += '<option value="'+res[i].cityId+'">'+res[i].city+'</option>';
		  }
		  $('#_city').html(html);
	   }
	}); 
...
function loadarea(pid){
	$.ajax({
	   type: "POST",
	   url: "/index.php?g=Shop&m=RecAddress&a=getarea",
	   data:"pid="+pid,
	   dataType:'json',
	   success: function(res){
		  var html = '<option value="">请选择</option>';
		  for(var i = 0; i< res.length; i++){
			   html += '<option value="'+res[i].areaId+'">'+res[i].area+'</option>';
		  }
		  $('#_area').html(html);
	   }
	}); 
...
window.myconfirm = function(title, success){
	myconfirm.sync = true;
	$('.confirm2 .msgtitle').html(title);
	$('.shadow').show();
	$('.confirm2').show();
	$('.confirm2 .btn1').click(function(){
		if(myconfirm.sync){
			myconfirm.sync = false;
			success();
		}
	})
	$('.confirm2 .btn2').click(function(){
		$('.shadow').hide();
		$('.confirm2').hide();			   
	})
}
window.myalert = function(title){
	$('.confirm .msgtitle').html(title);
	$('.shadow').show();
	$('.confirm').show();
...
function chooseseat(){
	...
	if(model == 1){  
		...
		if(dayid < 1){
			window.myalert('请选择日期');
			return;
		}
		if(ccid < 1){
			window.myalert('请选择场次');
			return;
		}
		window.location.href='/index.php?m=Yuyue&a=seat&activeid='+activeid+'&dayid='+dayid+'&ccid='+ccid+'&resultstr='+resultstr;
	}
}
//填写资料
function fullmsg(model){
	model = $('#model').val();
	activeid = $('#activeid').val();
	dayid = $('#dayid').val();
	ccid = $('#ccid').val();
	...
	numbox = $('#numbox').val();
	ordermoney = parseInt($('#ordermoney').val());
	if(model == 1){
		zwids = $('#zwids').val();
		if(dayid < 1){
			window.myalert('请选择日期');
		...
		if(zwids == ''){
			window.myalert('请选择座位');
			return;
		}
		window.location.href='/index.php?m=Confirm&a=index&activeid='+activeid+'&dayid='+dayid+'&ccid='+ccid+'&resultstr='+resultstr+'&orderstr='+orderstr+'&zwids='+zwids+'&ordermoney='+ordermoney+'&model='+model;
	}else if(model == 2){
		if(ccid < 1){
			window.myalert('请选择场次');
			return;
		}
		window.location.href='/index.php?m=Confirm&a=index&activeid='+activeid+'&dayid='+dayid+'&ccid='+ccid+'&resultstr='+resultstr+'&ordermoney='+ordermoney+'&numbox='+numbox+'&model='+model;
	}else if(model == 3){
		if(dayid < 1){
			window.myalert('请选择课程日期');
			return;
		}
		window.location.href='/index.php?m=Confirm&a=index&activeid='+activeid+'&dayid='+dayid+'&ccid='+ccid+'&resultstr='+resultstr+'&ordermoney='+ordermoney+'&numbox='+numbox+'&model='+model;
	}
...
function tuiejie(goodsid){
	if(goodsid < 0 ){
		alert('参数不正确'); return;	
	}
	$.ajax({
	   type: "POST",
	   url: "/index.php?g=Shop&m=Goods&a=dotj",
...
function commitdd(){
	...
	birth = '';
	isnull = false;
	$.each($("input[name='_babyname[]']"), function(k,v){
		if($(v).val() == ''){
			isnull = true;	
		}else{
			babyname+=$(v).val()+',';	
	...
	$.each($("input[name='_idcard[]']"), function(k,v){
		if($(v).val() == ''){
			isnull = true;	
		}else{
			idcard+=$(v).val()+',';	
	...
	$.each($("select[name='_sex[]']"), function(k,v){
		if($(v).val() == ''){
			isnull = true;	
		}else{
			sex+=$(v).val()+',';	
	...
	$.each($("input[name='_birth[]']"), function(k,v){
		if($(v).val() == ''){
			isnull = true;	
		}else{
			birth+=$(v).val()+',';	
	...
	if(babyname != ''){
		babyname = babyname.substr(0, babyname.length-1);	
	}
	if(idcard != ''){
	...
	if(parentname.trim().length < 2){
		window.myalert('请填写家长姓名');return;
	}
	re= /^(13[0-9]{9})|(18[0-9]{9})|(15[0-9]{9})|(17[0-9]{9})|(12[0-9]{9})|(16[0-9]{9})$/;
	if(!re.test(tel)){
		window.myalert('手机号格式不正确');return;
	}
	if(isnull){
		window.myalert('请填写完整表单信息');return;	
	}
	...
	$.ajax({
	   type: "POST",
	   url: "/index.php?&m=Confirm&a=checkout",
	   data:param,
	   dataType:'json',
	   success: function(res){
		  window.myalert(res.info);
		  if(res.status == 1){
			   window.location.href = "/index.php/Pay/index/model/"+model+"/order_no/"+res.data;
		  }
	   }
	});
}
...
function cancel(oid){
	window.myconfirm('确定取消订单？',function(){
		$.ajax({
		   type: "POST",
