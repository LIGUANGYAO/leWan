...
; (function ($) {
    $.extend($, {
        fileUpload: function (options) {
            var para = {
                multiple: false, // 设置多选的参数
                filebutton: ".filePicker",
                uploadButton: null,
                url: "/home/MUploadImg",                     //原图上传地址
                base64strUrl: "/home/MUploadImgBase64Str",   //压缩上传地址
                filename: "mfile",//mvc后台需要对应的名称
                maxSize:2,         //图片最大2 MB
                startCompress:0.01,   //图片大于1MB就启动压缩上传
                limitCount:3,      //最多10张图片
                auto: true,        //自动上传
                previewZoom: null,  //图片预览容器
                uploadComplete: function (res) {
                    //console.log("uploadComplete", res);
                    uploadCount++;
                    core.checkComplete(res);
                },
            ...
            var core = {
                fileSelected: function () {
                    ...
                    for (var i = 0; i < count; i++) {
                        if (i >= para.limitCount) {
                        	layer.msg("最多只能选择"+para.limitCount+"张图片!");
                            break;
                        }
                        var item = files[i];
                        ...
                        if (item.size > 1024 * 1024 * para.startCompress) {
                            console.log("图片大于"+para.startCompress+"M，开始进行压缩...");
                ...
                uploadBase64str: function (base64Str) {
                    ...
                    formdata.append(para.filename, base64Str);
                ...
                uploadFile: function (file) {
                    ...
                    formdata.append(para.filename, file);//这个名字要和mvc后台配合
                ...
                previewImage: function (res) {
        			...
        			divbox.setAttribute('className','uimgbox');
        			divbox.setAttribute('class','uimgbox');
        			
        			//2、创建图片
        			var imgnode = document.createElement('img');
        			imgnode.src = res.data.url;
        			
        			//3.删除
        			var drop = document.createElement('a');
        			drop.setAttribute('className','fa fa-trash-o');
        			drop.setAttribute('class','fa fa-trash-o');
        			...
        			input.setAttribute("name",para.inputhame);
        			input.setAttribute("value",res.data.filename);
        			
        			divbox.appendChild(imgnode);
        			divbox.appendChild(drop);
        			divbox.appendChild(input);
        			document.getElementById('preview').appendChild(divbox);
                }
            }
